...
def Write_ctl_file(file,model):
	...
	template_ctl_file = open('/bigrock_home/tkang/paml2022/ctl/'+species+'/paml_'+model+'.ctl')

	wf = open('/bigrock_home/tkang/paml2022/ctl/'+species+'/tempctl/temp_ctl_'+model+'.ctl','w')
...
def Run_codeml(model):
	...
	cmd = ['codeml', '/bigrock_home/tkang/paml2022/ctl/'+species+'/tempctl/temp_ctl_'+model+'.ctl']
	sp.call(cmd)
