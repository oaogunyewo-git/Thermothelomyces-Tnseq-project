...
MySpeciesList = ['apoper-1', 'canare-1', 'chacoc-1','chathe-1', 'corsep-1', 'crahot-1', 'crathe-1', 'dicfun-1', 'gelunc-1', 'mycthe-1', 'neudis4-001b', 'parapp-1', 'parhyr-1', 'podans-001', 'podapp-1', 'podcur-1', 'thehet-1', 'theter-1', 'theher-1', 'thethe-1', 'triant-1']	# MAKE A LIST OF SPECIES NAMES YOU ACTUALLY ARE INTERESTED IN
...
with open('Orthogroups.txt') as proj3file: # the ''   here makes this a string
		...
		if my_orthogroup_no_colon in single_copy_ortho:
			...
			# count how many species of interest are in this orthogroup
			n_overlap = 0
			for gene in genes_in_my_orthogroup:
				ret = gene.split('_')		# strip off gene name and just leave species name
				gene_name_stripped = ret[0]	# split the string!!!
				if gene_name_stripped in MySpeciesList:
					n_overlap = n_overlap + 1
			
			if(n_overlap <> 20):	# if n_overlap isn't 20, give up and go on to the next orthogroup
				break
			else:
				print(n_overlap)	
			...
			for gene in genes_in_my_orthogroup:
				...
				if(sp_name in MySpeciesList):  			# PUT IN AN IF-IN STATEMENT TO ASK IF SP_NAME IS AMONG THOSE OF INTEREST
					my_sp_filename = 'individuals_fastas_CDS_aa/' + sp_name + '.fasta' 	#new string of letters which is the sp_name.fasta; rem to provide the directory for it to be read
