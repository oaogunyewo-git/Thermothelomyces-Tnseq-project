...
def allow_mismatches(seq1, seq2, max_mismatches):
...
def main(argv):
    ...
    parser.add_argument("-q", "--qual", dest="minQual", help="Minimum quality score for the barcode region for a read to counted. Default is 5",default=5, type=int)  # Reduced quality threshold
    parser.add_argument("-b", "--matchBefore", dest="matchBefore", help="Number of bases before the barcode to match. Default is 4", default=4, type=int)  # Reduced matching criteria
    parser.add_argument("-a", "--matchAfter", dest="matchAfter", help="Number of bases after the barcode to match. Default is 4", default=4, type=int)  # Reduced matching criteria
    ...
    parser.add_argument("--maxMismatches", dest="maxMismatches", help="Maximum allowed mismatches for barcode matching. Default is 1", default=1, type=int)  # New parameter for mismatches
    ...
    if not (poolFileName == "NO INPUT"):
        try:
            with open(poolFileName, 'rb') as poolFileHandle:
                poolFrame = pd.read_csv(poolFileHandle,low_memory=False,dtype={'NearestGene':str,'CodingFraction':str})
    ...
    if poolFileFound:
        poolCounts = poolFrame[['rcbarcode','scaffold','pos','gene']].copy()
    ...
    if (not options.quietMode):
        statusUpdate = 'Finding barcodes in fastqs and counting occurrences'
        printUpdate(options.logFile,statusUpdate)
        statusUpdate = "---------------------"
        printUpdate(options.logFile,statusUpdate)

    # List to hold barcodes found in BarSeq but not in TnSeq poolfile
    barcodes_not_in_poolfile = []
    ...
    for fileIndex in fileIndexes:
        ...
        if  indexRow['UsePrecounted']:
                            ...
                            if passIndex:
                                ...
                                if matchedBefore > -1:
                                    ...
                                    while n < len(allowedLengths) and not compliantBarcode:
                                        ...
                                        if searchAfter > 1 and allow_mismatches(afterbarcode[:searchAfter], endSearch[:searchAfter], options.maxMismatches):
            ...
            if matchAfterWarning:
                statusUpdate = "    " + str(matchAfterWarning) + " reads were too short to check " + str(options.matchAfter) + " bases after barcodes as specified with --matchAfter.  For these reads all remaining bases will be checked against the expected sequence. Reads with less than two remaining bases after the barcode will be counted as non-compliant." 
                printUpdate(options.logFile,statusUpdate)
            ...
            if (not options.quietMode):
                ...
                printUpdate(options.logFile,statusUpdate)
                statusUpdate = "      " + str(noPostSeq) + " reads without expected sequence after the barcode region."
                printUpdate(options.logFile,statusUpdate)
                statusUpdate = "      " + str(nonCompliantBarcode) + " reads with noncompliant barcodes. (Contains Ns, etc)."
                printUpdate(options.logFile,statusUpdate)
        ...
        if poolFileFound:
            ...
            # Find barcodes in BarSeq but not in TnSeq poolfile
            barcodes_in_pool = set(poolCounts['barcode'])
            barcodes_not_in_poolfile.extend([barcode for barcode in barcodeCounts if barcode not in barcodes_in_pool])
        else:
            barcodeCountsFrame = pd.DataFrame.from_dict(barcodeCounts, orient='index',columns=[fileIndex],dtype=int)
        ...
        threes = (barcodeCountsFrame[fileIndex] >= 3).sum()
        ...
        # Save barcodes seen three times or more
        barcodes_seen_three_or_more = barcodeCountsFrame[barcodeCountsFrame[fileIndex] >= 3]
        barcodes_seen_three_or_more_file = outputDir + "barcodes_seen_three_or_more.txt"
        barcodes_seen_three_or_more.to_csv(barcodes_seen_three_or_more_file, index=True, sep='\t')
        statusUpdate = f"Barcodes seen three times or more written to {barcodes_seen_three_or_more_file}"
        printUpdate(options.logFile, statusUpdate)
    ...
    if poolFileFound:
        ...
        # Write barcodes not in poolfile to a new file
        if barcodes_not_in_poolfile:
            barcodes_not_in_poolfile_df = pd.DataFrame(barcodes_not_in_poolfile, columns=['Barcode'])
            barcodes_not_in_poolfile_file = outputDir + "barcodes_not_in_poolfile.txt"
            barcodes_not_in_poolfile_df.to_csv(barcodes_not_in_poolfile_file, index=False, sep='\t')
            statusUpdate = f"Barcodes found in BarSeq but not in TnSeq poolfile written to {barcodes_not_in_poolfile_file}"
            printUpdate(options.logFile, statusUpdate)
        else:
            statusUpdate = "All barcodes found in BarSeq were also found in TnSeq poolfile."
            printUpdate(options.logFile, statusUpdate)
