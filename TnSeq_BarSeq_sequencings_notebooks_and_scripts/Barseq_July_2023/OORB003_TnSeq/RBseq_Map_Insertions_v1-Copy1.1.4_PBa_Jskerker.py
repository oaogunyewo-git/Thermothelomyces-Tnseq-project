...
#this code was modified from RBseq_Map_Insertions_v1.1.4 from the RBseq repository of Scoradetti by Jskerker

tn_pattern = regex.compile('(?e)(GATCCAAACTATCAGTGTTTGA){e<=2}')  # last 22 bps of the right arm of the transposon.  searches for this pattern allowing 2 mismatchs for sequencing errors

Version = '1.1.4_PBa'
ReleaseDate = 'August 8, 2020'

# piggyBAC version - changed matchJunction default = 4
# added line 641 to print out offbyOne
...
def main(argv):
    ...
    parser.add_argument("-N", "--filterNeighborhood", dest="filterNeighborhood", help="Number of basepairs around each insertion to filter out less abundant insertions with similar barcodes. Default is 10", default=10, type=int)
    ...
    if (not fileToOpen):
        ...
        sys.exit()
    try:
        with open(fileToOpen, 'r', encoding='UTF-8') as FileHandle:
    ...
    if options.useMappedFiles:
        ...
        for fastqNum,fastqFile in enumerate(fastqFiles):
            ...
            try:
                ...
                with open(blastQueryFileName, 'w', encoding='UTF-8') as blastQueryFile:
            ...
            try:
                ...
                with open(noBarcodeFoundFileName, 'w', encoding='UTF-8') as noBarcodeFile:
                ...
                with open(fileToOpen, 'r', encoding='UTF-8') as FileHandle:
                    ...
                    for readLine in FileHandle:
                        ...
                        #Third line after a readname is the quality score, now weve loaded info for read and can filter for barcode presence before moving to next read.
                            ...
                            #If the barcode is found, check for end of Tn
                            if compliantBarcode:
                                NbarcodeFound+=1
                                tn_match_data = tn_pattern.search(readSeq)  # searches for the tn pattern
                                if tn_match_data == None:
                                    continue
                                else:
                                    genomeQuery = readSeq[tn_match_data.end() - 4:]
                                    if len(genomeQuery) < 40:
                                        continue
                                ...
                                if len(QueryWriteBlock) > 10000:
                                    ...
                                    try:
                                        with open(blastQueryFileName, 'a', encoding='UTF-8') as blastQueryFile:
                            ...
                            #Save reads without barcodes for troubleshooting TnSeq
                                ...
                                if len(NoBarcodeFoundWriteBlock) > 10000:
                                    ...
                                    try:
                                        with open(noBarcodeFoundFileName, 'a', encoding='UTF-8') as noBarcodeFile:
            ...
            try:
                BlastOutputFile = outputDirs[fastqNum]+shortNames[fastqNum]+"_blastGenome.txt"
                with open(BlastOutputFile, 'r', encoding='UTF-8') as BlastOutput:
            ...
            try:
                ...
                with open(mappedReadsFileName, 'w', encoding='UTF-8') as mappedReadsFile:
    ...
    for pool in mappedFilesPerPool:
        ...
        for tabFileName in mappedFilesPerPool[pool]:
            ...
            printUpdate(options.logFile,statusUpdate)
            try:
                with open(tabFileName, 'r', encoding='UTF-8') as tabFileHandle:
...
#                         printUpdate(options.logFile, f'{variantBarcode}\t{barcode}')

                        offByOneList.append(barcode)

        outPutFileName = poolOutPutDirs[pool] + pool + "_poolfile"
        offByOneFileHandle = open(outPutFileName+"_offByOne.txt", 'w', encoding='UTF-8')
        ...
        statusUpdate = "    Wrote " + str(nOffByOne) + " off-by-one barcodes ("+str(OffByOneReads)+" reads) to "+outPutFileName+"_offByOne.txt"
        printUpdate(options.logFile,statusUpdate)

        #Set up pool file
        
        outPutFileHandle = open(outPutFileName, 'w', encoding='UTF-8')
        ambFileHandle = open(outPutFileName+"_ambiguous.txt", 'w', encoding='UTF-8')
        multiFileHandle = open(outPutFileName+"_multiLocus.txt", 'w', encoding='UTF-8')
        ...
        statusUpdate = "    Wrote "+str(nMulti)+" barcodes ("+str(MultiReads)+" reads) to "+outPutFileName+"_multiLocus.txt"
        printUpdate(options.logFile,statusUpdate)
        multiFileHandle.close()

        statusUpdate = "    Wrote "+str(nAmb)+" barcodes ("+str(AmbReads)+" reads) to "+outPutFileName+"_ambiguous.txt"
        printUpdate(options.logFile,statusUpdate)
        ambFileHandle.close()

        #Set up pool file for insert hits
        outPutFileHandle = open(outPutFileName+"_insertReadsOnly.txt", 'w', encoding='UTF-8')
        ...
        statusUpdate = "    Wrote "+str(nInsert)+" barcodes ("+str(allInsertReads)+" reads) to "+outPutFileName+"_insertHitsOnly.txt"
        printUpdate(options.logFile,statusUpdate)


        if options.filterNeighborhood > 0 and len(barcode_primary_location) > 0:
            #Find similar sequences (off-by-two, short indels) at same position (computationaly intensive).
            statusUpdate = "  Finding similar barcodes that mapped to the same location (off-by-two, indels, etc)."
            printUpdate(options.logFile,statusUpdate)
            ...
            for scaffold, barcodes in scaffoldGroups:
                ...
                #Scroll a moving window through each scaffold checking local neighborhood for similar barcodes
                for idx, ref_position in enumerate(scaffold_barcode_positions[0:-1]):
                    ...
                    while check_idx < len(scaffold_barcode_positions) and scaffold_barcode_positions[check_idx] - ref_position < int(options.filterNeighborhood):
                        ...
                        #Calculate minimum edit distance for nearby by barcodes
                            ...
                            # filterTest = (int(options.filterEditDistance) > levenshtein(scaffold_barcode_list[idx], scaffold_barcode_list[check_idx]) > 1)
            ...
            outPutFileHandle = open(outPutFileName, 'w', encoding='UTF-8')
            obtFileHandle = open(outPutFileName+"_filteredLocal.txt", 'w',encoding='UTF-8')
            barcodeSummary = "\t".join(["barcode","rcbarcode","nTot", "n", "scaffold", "strand", "pos","type","nMainLocation", "nInsert", "All genomic mappings", "All insert mappings"])
